...
interface LoginFormProps {
  /** Live count of active founders, or null when unavailable. */
  founderCount: number | null;
}

export function LoginForm({ founderCount }: LoginFormProps) {
  ...
  // Real numbers only: the founder count comes from the database; the other
  // panels are factual brand statements, not metrics.
  const stats =
    founderCount !== null && founderCount > 0
      ? [
          {
            value: founderCount.toLocaleString("en-CA"),
            label: "Founding Members",
          },
          { value: "Canada", label: "Born and based" },
          { value: "Lifetime", label: "Member access" },
        ]
      : [
          { value: "Founders", label: "First cohort forming" },
          { value: "Canada", label: "Born and based" },
          { value: "Lifetime", label: "Member access" },
        ];
